    ...
    */
    ["move", function (panelNumber) {
        ...
        var Text1 = this.add.dom(
            (SCREEN_WIDTH/2 + 570 * panelNumber) - GRID * 2.5, 
            GRID * 19, 'div',  
        ...
        var tutWASD = this.add.sprite(SCREEN_WIDTH/2 + GRID * 6.5,
            SCREEN_HEIGHT/2 + GRID  * 4.25
        ).setDepth(105).setOrigin(0.5,0.5);
        tutWASD.play('tutAll').setAlpha(0)

        var tutSnake = this.add.sprite(SCREEN_WIDTH/2,
    ...
    ["atoms", function (panelNumber) {
        ...
        var tutAtomSmall = this.add.sprite((SCREEN_WIDTH/2 + hOffSet * 1) - GRID * 3,
        SCREEN_HEIGHT/2 + GRID  * .5).setDepth(103).setOrigin(0.5,0.5);
        tutAtomSmall.play('atom04idle');
        
        var tutAtomMedium = this.add.sprite((SCREEN_WIDTH/2 + hOffSet * 1) - GRID * 1,
            SCREEN_HEIGHT/2 + GRID  * .5).setDepth(103).setOrigin(0.5,0.5);
        tutAtomMedium.play('atom03idle');
        
        var tutAtomLarge = this.add.sprite((SCREEN_WIDTH/2 + hOffSet * 1) + GRID * 1,
            SCREEN_HEIGHT/2 + GRID  * .5).setDepth(103).setOrigin(0.5,0.5);
        tutAtomLarge.play('atom02idle');
        
        var tutAtomCharged = this.add.sprite((SCREEN_WIDTH/2 + hOffSet * 1) + GRID * 3,
            SCREEN_HEIGHT/2 + GRID  * .5).setDepth(103).setOrigin(0.5,0.5);
        tutAtomCharged.play('atom01idle');
        
        var tutAtomElectrons = this.add.sprite((SCREEN_WIDTH/2 + hOffSet * 1) + GRID * 3,
            SCREEN_HEIGHT/2 + GRID  * .5).setDepth(103).setOrigin(0.5,0.5);
    ...
    ["portals", function (panelNumber) {
        ...
        var tutPortal1 = this.add.sprite((SCREEN_WIDTH/2 + hOffSet * 2) - GRID * 2,
        SCREEN_HEIGHT/2 - GRID  * 1).setDepth(103).setOrigin(0.5,0.5);
        
        tutPortal1.play('portalIdle');
        var tutPortal2 = this.add.sprite((SCREEN_WIDTH/2 + hOffSet * 2) + GRID * 2,
            SCREEN_HEIGHT/2 + GRID  * 1).setDepth(103).setOrigin(0.5,0.5);
